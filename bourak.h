...
// ═══════════════════════════════════════════════════════════════
void setupMPU() {
    I2CMPU.beginTransmission(0x68); I2CMPU.write(0x6B); I2CMPU.write(0x00); I2CMPU.endTransmission();
    I2CMPU.beginTransmission(0x68); I2CMPU.write(0x1B); I2CMPU.write(0x08); I2CMPU.endTransmission();
    I2CMPU.beginTransmission(0x68); I2CMPU.write(0x1C); I2CMPU.write(0x00); I2CMPU.endTransmission();
}
...
void recordAccelRegisters() { recordMPURegisters(0x3B, 6, &accelX, &accelY, &accelZ); processAccelData(); }
void recordGyroRegisters()  { recordMPURegisters(0x43, 6, &gyroX,  &gyroY,  &gyroZ);  processGyroData();  }
...
// calcANG optimisé : lecture Z uniquement (2 bytes, repeated start)
// ~170µs au lieu de ~350µs → gain x2 sur la boucle principale
IRAM_ATTR void calcANG() {
    ...
    // Lire seulement les 2 bytes GyroZ (0x47-0x48) avec repeated start
    I2CMPU.beginTransmission(0x68);
    I2CMPU.write(0x47);
    I2CMPU.endTransmission(false); // repeated start
    I2CMPU.requestFrom(0x68, (uint8_t)2);
    unsigned long t = micros() + 500;
    while (I2CMPU.available() < 2 && micros() < t);
    if (I2CMPU.available() >= 2) {
        gyroZ = (I2CMPU.read() << 8) | I2CMPU.read();
    }
